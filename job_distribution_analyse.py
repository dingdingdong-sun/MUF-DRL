...
load1 = Workloads("./data/HPC2N-2002-2.2-cln.swf")

load2 = Workloads("./data/SDSC-BLUE-2000-4.2-cln.swf")

load3 = Workloads("./data/CTC-SP2-1996-3.1-cln.swf")
...
def plot_job_distribution(requests_resource, job_size, sub,t):
    ...
    ax = fig.add_subplot(1, 3, sub, projection='3d')  # 三维坐标轴
...
#     count += 1
for j in load1.all_jobs:
    requests_resource1.append(j.request_number_of_processors)
    job_size1.append(j.run_time)
    count += 1

for j in load2.all_jobs:
    requests_resource2.append(j.request_number_of_processors)
    job_size2.append(j.run_time)

for j in load3.all_jobs:
    requests_resource3.append(j.request_number_of_processors)
    job_size3.append(j.run_time)
...
plot_job_distribution(requests_resource1,job_size1,1, "HPC2n")
plot_job_distribution(requests_resource2,job_size2,2, "SDSC-BLUE")
plot_job_distribution(requests_resource3,job_size3,3, "CTC-SP2")
